...
class DotfileMap:
    ...
    def create_symlink(self, source: Path, target: Path):
        ...
        if target.is_symlink() or target.exists():
            ...
            if self.sudo:
                subprocess.run(["sudo", "mv", str(target), str(backup_path)], check=True) # pyright: ignore[reportUnusedCallResult]
            else:
        ...
        try:
            if self.sudo:
                subprocess.run(["sudo", "ln", "-sf", str(source), str(target)], check=True) # pyright: ignore[reportUnusedCallResult]
            else:
...
def main():
    mappings = [
        ...
        DotfileMap(source_name="cyrene-overlay", target_name=Path("/var/db/repos/cyrene-overlay"), sudo=True)
    ]
    print("🚀 开始链接 Dotfiles...")
    for m in mappings:
        m.sync()
